...
namespace Venz.Telemetry
{
    public sealed class DiagnosticsLevel
    {
        ...
        [DefaultOverload]
        public void Log(String customText, Object parameter) => Log(customText, "Diagnostics", parameter);

        public void Log(String customText, String parameterName, Object parameterValue)
        {
            lock (Sync)
                foreach (var service in TelemetryServices)
                    service.LogEvent($"{ProcessName} || {customText}", parameterName, parameterValue.AsJsonString());
        }
